...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    last, best = w / 'last.pt', w / 'best1.pt' #最后一轮和最好的一轮的权重文件
    ...
    if cuda and RANK == -1 and torch.cuda.device_count() > 1:
        LOGGER.warning('WARNING: DP not recommended, use torch.distributed.run for best DDP Multi-GPU results.\n'
                       'See Multi-GPU TuYolov5代码详解torial at https://github.com/ultralytics/yolov5/issues/475 to get started.')
...
def main(opt, callbacks=Callbacks()):
    ...
    # Checks
    if RANK in [-1, 0]:
        print_args(FILE.stem, opt)  # 打印文件的参数信息
        check_git_status() #检查框架的代码是否更新
        check_requirements(exclude=['thop'])

    # Resume
    #Resume参数 主要是用于训练中的中断问题 若训练中中断 可以传入参数 接着训练
    ...
    # DDP mode
    device = select_device(opt.device, batch_size=opt.batch_size) #训练的参数设定
    if LOCAL_RANK != -1:
    ...
    if not opt.evolve:
        ...
        if WORLD_SIZE > 1 and RANK == 0:  #分布式训练
